// const MOCK = true;
const MOCK = (location.href.search('localhost') != -1 ||
...
function compToLinear(comp) {
    return Math.pow(comp / 255, invGamma);
}
let last = undefined;
function setColorWs(color, websocketsConn) {
    if (color == last) {
...
function matrixMult(matrix, vec) {
...
// WEBSOCKETS STUFF
function getWebsocketsConn(onopen) {
    if (MOCK) {
...
function getWebsocketUrl(localUrl) {
...
function checkConnOpen(websocketsConn) {
