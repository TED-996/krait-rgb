...
function onCmdFade1() {
  ...
  setTimeout(function() {
    cmdFadeLoop(colorsLinear as ColorList, Date.now() / 1000.0, false)
  }, 0);
}
...
function onCmdFadeRepeat() {
  ...
  colorsLinear = colorsLinear as ColorList;

  colorsLinear.push(colorsLinear[0]);

  setTimeout(function() {
    cmdFadeLoop(colorsLinear as ColorList, Date.now() / 1000.0, true)
  }, 0);
}
...
function onCmdBpm() {
  ...
  setTimeout(function() {
    cmdBpmLoop(colorsLinear as ColorList, bpm, Date.now() / 1000.0);
  }, 0);
}
...
function toColorListLinear(lines : string[]) : ColorList|false {
  if (lines.length == 0) {
  ...
  if (colors
          .filter(function(c : string | false) {
            return c === false
          })
          .length != 0) {
    // some parsing errors
  ...
  let colorsChecked = colors as string[];
  let colorsLinear = colorsChecked.map(function(c) {
    return hexToRgb(c).map(compToLinear);
  });
  return colorsLinear as ColorList;
}

function getBlendedInColors(colorsLinear : ColorList, progress : number) {
  let r = 0;
  let g = 0;
  let b = 0;
...
function cmdFadeLoop(colorsLinear : ColorList, startTime : number, repeat : boolean) {
  if (stopRequested) {
...
function cmdBpmLoop(colorsLinear : ColorList, bpm : number, startTime : number) {
  if (stopRequested) {
