...
let lastMusicSource : AudioBufferSourceNode;
let analyserNode : AnalyserNode;

enum MusicStatus { Playing, Paused, Stopped };
let musicStatus : MusicStatus = MusicStatus.Stopped;
...
function onMusicPlay(){
  if (musicStatus == MusicStatus.Stopped){
    ...
    connPromise.then(
      function() { return getMusicBuffer(context)},
      function(reason) { addOutLine("Cannot connect to RGB server. Reason: " + reason)}
    ).then(function(musicBuffer : AudioBuffer | undefined) {
      if (typeof(musicBuffer) === "undefined"){
        return;
      }
      let musicSource = getMusicSource(context, musicBuffer as AudioBuffer);
...
declare let webkitAudioContext : new() => AudioContext;

function getAudioContext(lastSource? : {context : AudioContext}|undefined) : AudioContext {
  ...
  if ("webkitAudioContext" in window){
    return (new webkitAudioContext()) as AudioContext;
  }
  throw new Error("Web Audio not supported.");
}

function getMusicSource(context : AudioContext, musicBuffer : AudioBuffer){
  let musicSource = context.createBufferSource();
  musicSource.buffer = musicBuffer;
  return musicSource;
}

function getAnalyser(context : AudioContext){
...
function registerTimeDisplays(context : AudioContext){
...
let lastRgb : Color3 = undefined;

function registerAnalyser(context : AudioContext){
  if (analyserRegistered){
    return;
  }
  analyserRegistered = true;

  let canvas = $("#music-visualizer")[0] as HTMLCanvasElement;
...
function computeRgbFromFft(fftBuffer : Uint8Array) : [number, number, number]{
...
function rgbToCssValue(color : Color3){
  return "rgb(" + color[0].toString() + "," + color[1].toString() + "," + color[2].toString() + ")"
}

function getPoly2FuncFromRoots(root1 : number, root2 : number, max : number){
...
function getMusicBuffer(context : AudioContext) : Promise<AudioBuffer|undefined> {
  let input = $("#music-file-input");
  let asInput = input[0] as HTMLInputElement;
  ...
  return new Promise(function(
      resolve : (value : AudioBuffer|undefined) => void,
      reject : (reason : any)=>void) {
    let file = files[0];
    let reader = new FileReader();
    reader.onload = function(e){
      let result = (e.target as FileReader).result as ArrayBuffer;
